...
/*
  ================================================================================
    ...
    Description: This version is capable of reading both temperature and humidity inside the chamber
...
const double BALANCE_RESISTOR   = 9999;//9976.1; // Measured on the PCB (1% resistor used)
...
DHT dht(DHTPIN, DHTTYPE);   // Instantiate Humidity sensor
Keypad customKeypad = Keypad(makeKeymap(hexaKeys), rowPins, colPins, ROWS, COLS); // Instantiate keypad
LiquidCrystal_I2C lcd(0x3F, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE); //0x3F 0x27 Instantiate LCD screen
...
void setup() {
  ...
  // Used by the incremental encoder (side knob) for changing values
  // **
  //                                **DO NOT TOUCH!!!!!
  //**
...
// Interupt routine when a change on the encoder occurs
ISR(PCINT0_vect) {
  // flash(10);
